...
const RoomApp: React.FC = () => {
    ...
    const generatingItemRef = useRef<string | null>(null); // 单件家具懒生成的 in-flight 防抖（防连点双发）
    ...
    // 生活三层派生链（docs/life-layers-design.md）：「角色此刻在做什么」直接读日程当前 slot，
    // 零 LLM 调用。slot 判定与 context.ts buildScheduleInjection 同款分钟数比较。
    const getCurrentSlotHint = (schedule: DailySchedule | null): string | null => {
        ...
        for (let i = schedule.slots.length - 1; i >= 0; i--) {
            ...
            if (cur >= h * 60 + m) {
                ...
                return `${s.emoji ? s.emoji + ' ' : ''}正在${s.activity}${s.location ? `（${s.location}）` : ''}`;
            }
        }
        return null;
    };
    ...
    const handleEnterRoom = async (c: CharacterProfile) => {
        ...
        // 家具描述是**永久缓存**（点哪件生成哪件、跨天保留，家具被删才作废），
        // 不再按天丢弃——这是砍掉每日全量生成的前提。
        ...
        if (hasCache && c.savedRoomState) {
            ...
            // 不在进门时阻塞生成——直接进屋。今日的计划/随笔交给「更新这一天」；
            // 「ta 此刻在做什么」直接读日程当前 slot，进门即有状态、零调用。
            const slotHint = getCurrentSlotHint(existingSchedule);
            setAiBubble(slotHint ? { text: slotHint, visible: true } : { text: '', visible: false });
        }
    };
                ...
                try {
                    ...
                    updateCharacter(c.id, {
                        ...
                        savedRoomState: {
                            ...
                            // 家具描述走懒生成永久缓存，fallback 不再填占位描述覆盖它
                            items: c.savedRoomState?.items || {},
                            actorAction: 'idle'
                        }
                    });
    ...
    const initializeRoomState = async (c: CharacterProfile, currentItems: RoomItem[], force: boolean = false) => {
        ...
        const loadingTexts = [`正在走进${c.name}的一天...`, "正在整理思绪...", "正在写今天的计划...", "正在随笔本上涂涂写写..."];
        let textIdx = 0;
        ...
        try {
            ...
            // 「此刻在做什么」读日程当前 slot（派生链），不再让这次调用去猜
            const slotHint = getCurrentSlotHint(existingSchedule);

            // 物品交互已改为「点哪件懒生成哪件 + 永久缓存」（见 handleLookAt），
            // 这里不再批量生成全部物品——那是旧长加载「物件越多越久」的来源。
            let prompt = `${baseContext}

### [Environment Context - Critical]
**当前现实时间**: ${nowDateStr} ${nowTimeStr}
**与用户上次互动距离现在**: ${timeGapHint}
${slotHint ? `**你此刻的日程状态**: ${slotHint}\n` : ''}**最近互动记录 (Latest 50)**:
${chatContext}

### [今日房间状态]
用户进入了**你的**房间。请生成你迎接 ta 的这一刻，以及（如果需要）你今天的计划和随笔。

### 1. 欢迎语 (Welcome)
- **Welcome**: 看到用户进来，你第一句话说什么？(请结合时间差：如果很久没见，是惊讶、想念还是生气？${slotHint ? '也可以自然带上你此刻正在做的事。' : ''})
...
`;
            ...
            if (response.ok) {
                ...
                updateCharacter(c.id, {
                    lastRoomDate: todayStr,
                    savedRoomState: {
                        // actorStatus 来自日程当前 slot（派生链），不再由 LLM 猜
                        actorStatus: slotHint || '',
                        welcomeMessage: result.welcomeMessage,
                        // 家具描述是永久缓存（懒生成积累），每日更新不清空
                        items: c.savedRoomState?.items || {},
                        actorAction: 'idle'
                    }
                });
    ...
    // 懒生成单件家具的描述与反应：点哪件生成哪件，之后永久缓存（家具被删才作废）。
    // 取代旧的「更新这一天」批量生成全部物品——那是长加载的最大来源。
    const generateItemInteraction = async (c: CharacterProfile, item: RoomItem): Promise<ItemInteraction | null> => {
        try {
            const baseContext = ContextBuilder.buildCoreContext(c, userProfile, false);
            const slotHint = getCurrentSlotHint(roomSchedule);
            const prompt = `${baseContext}

### 场景
用户「${userProfile.name}」走进了你的房间，凑近看你的「${item.name}」。${slotHint ? `你此刻的状态：${slotHint}。` : ''}
${item.descriptionPrompt ? `这件物品的背景设定：${item.descriptionPrompt.substring(0, 200)}` : ''}

请生成：
- description: 旁白视角的物品外观/状态描写（1-2句，可带感官细节）。
- reaction: 用户查看时你（${c.name}）的吐槽或反应（1句，口语、贴人设）。

严格输出 JSON：{ "description": "...", "reaction": "..." }`;
            ...
            return { description: String(parsed.description), reaction: String(parsed.reaction || '…') };
        } catch (e) {
            console.error('[Room] item lazy-gen failed:', e);
            return null;
        }
    };

    /** 把一次物品互动落进聊天（与角色共享这段经历），带最近 50 条去重。 */
    const logItemObservation = async (c: CharacterProfile, itemName: string, cached: ItemInteraction) => {
        const contentToCheck = `[${userProfile.name}]在[${c.name}]的${itemName}上看到了：${cached.description}。[${c.name}]表示：${cached.reaction}`;
        const recentMsgs = await DB.getMessagesByCharId(c.id);
        const isDuplicate = recentMsgs.slice(-50).some(m => m.role === 'system' && m.content === contentToCheck);
        if (!isDuplicate) {
            try {
                await DB.saveMessage({ charId: c.id, role: 'system', type: 'text', content: contentToCheck });
            } catch (err) {}
        }
    };
    ...
    const handleLookAt = async (item: RoomItem, e?: React.MouseEvent) => {
        ...
        if (cached) {
            ...
            await logItemObservation(char, item.name, cached);
        } else if (item.isInteractive && apiConfig.apiKey && generatingItemRef.current !== item.id) {
            // 懒生成：第一次点这件家具时现场生成一次，之后永久缓存、点击零等待
            generatingItemRef.current = item.id;
            setObservationText(`（凑近看看${item.name}……）`);
            setAiBubble({ text: '(让我看看…)', visible: true });
            try {
                const generated = await generateItemInteraction(char, item);
                if (generated) {
                    const next = { ...roomDescriptions, [item.id]: generated };
                    setRoomDescriptions(next);
                    updateCharacter(char.id, {
                        savedRoomState: {
                            ...(char.savedRoomState || { actorStatus: '', welcomeMessage: '', items: {} }),
                            items: next,
                        }
                    });
                    setObservationText(generated.description);
                    setAiBubble({ text: generated.reaction, visible: true });
                    await logItemObservation(char, item.name, generated);
                } else {
    ...
    const deleteSelectedItem = () => {
        if (!selectedItemId) return;
        saveRoom(items.filter(i => i.id !== selectedItemId));
        // 家具删除 = 它的永久描述缓存一并作废（懒生成缓存的唯一失效时机）
        if (char?.savedRoomState?.items?.[selectedItemId]) {
            const { [selectedItemId]: _gone, ...rest } = char.savedRoomState.items;
            updateCharacter(char.id, { savedRoomState: { ...char.savedRoomState, items: rest } });
        }
        setRoomDescriptions(prev => { const n = { ...prev }; delete n[selectedItemId]; return n; });
        setSelectedItemId(null);
    };
    ...
    return (
        <div className="h-full w-full bg-[#f8fafc] flex flex-col relative overflow-hidden font-sans select-none">
            ...
            {/* 「更新这一天」现在只生成欢迎语 + 今日计划 + 随笔（一次小调用）。
                物品交互改为点哪件懒生成哪件并永久缓存（见 handleLookAt），
                「ta 此刻在做什么」直接读日程当前 slot——旧的整屋批量长加载已移除。 */}
            {isInitializing && (
                <div className="absolute inset-0 z-[500] bg-white flex flex-col items-center justify-center animate-fade-in px-10 text-center">
                    ...
                    <p className="text-[11px] text-slate-400/90 leading-[1.7] mt-3 max-w-[268px]">
                        正在生成 ta 迎接你的这一刻、今天的计划与随笔——很快。
                        <br />屋里的物品不用等：点到哪件，才会在那一刻生成它的故事，之后永久记住。
                    </p>
                </div>
            )}
            ...
            {mode === 'view' && !todayGenerated && !isInitializing && !observationText && (
                <div className="absolute bottom-6 left-4 right-4 bg-white p-4 rounded-2xl shadow-2xl border border-slate-100 z-[150] animate-slide-up flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-slate-400 leading-snug mt-0.5">生成今天的问候、计划与随笔（很快）。物品反应点到哪件才生成哪件。</p>
                    </div>
