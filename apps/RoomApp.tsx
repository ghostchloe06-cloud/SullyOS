...
import { RoomItem, CharacterProfile, RoomTodo, RoomNote, DailySchedule, AppID, Message } from '../types';
...
const RoomApp: React.FC = () => {
    ...
    // Core State
    const [viewState, setViewState] = useState<'select' | 'dashboard' | 'room' | 'pixelHome'>('select');
    // 看板「消息」卡的最近聊天预览
    const [dashMessages, setDashMessages] = useState<Message[]>([]);
    ...
    const handleEnterRoom = async (c: CharacterProfile) => {
        ...
        // 先进「角色看板」（Gotchi 主界面），LIVE 窗点进去才是全屏小屋
        setViewState('dashboard');

        // 看板消息卡：最近几条文本消息预览（异步补上，不阻塞进门）
        DB.getMessagesByCharId(c.id).then(msgs => {
            setDashMessages(msgs.slice(-12));
    ...
    useEffect(() => {
        ...
        if (c) {
            ...
            setViewState('room'); // 梦境深链直达全屏小屋（handleEnterRoom 默认落看板）
            setShowDream(true);
        }
...
`;
    ...
    // 角色看板（Gotchi 主界面）：进小屋的第一屏。放在全部 hooks 之后，
    // 与 room 视图切换时 hook 顺序保持一致。
    if (viewState === 'dashboard' && char) {
        return (
            <RoomDashboard
                char={char}
                items={items}
                ...
                welcomeMessage={todayGenerated ? char.savedRoomState?.welcomeMessage : undefined}
                recentMessages={dashMessages}
                interaction={{
                    // 与全屏小屋共用同一份交互 state/handler：LIVE 窗就是小屋本体
                    actorState,
                    aiBubble,
                    onLookAt: handleLookAt,
                    onPokeActor: handlePokeActor,
                    onDismissBubble: () => setAiBubble(prev => ({ ...prev, visible: false })),
                }}
                observationText={observationText}
                onDismissObservation={() => setObservationText('')}
                onBack={() => setViewState('select')}
                onEnterRoom={() => setViewState('room')}
                onOpenChat={() => openApp(AppID.Chat)}
                onOpenFragments={() => { setViewState('room'); setShowSidebar(true); }}
                onOpenSettings={() => { setViewState('room'); setMode('edit'); setShowSettingsModal(true); }}
            />
        );
    }
    ...
    return (
        <div className="h-full w-full bg-[#f8fafc] flex flex-col relative overflow-hidden font-sans select-none">
            ...
            <div className="absolute top-0 w-full px-4 pb-2 flex justify-between z-30 pointer-events-none" style={{ paddingTop: 'max(3rem, var(--safe-top, 0px))' }}>
                <button onClick={() => setViewState('dashboard')} className="bg-white/90 p-2 rounded-full shadow-md pointer-events-auto active:scale-90 transition-transform text-slate-600"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" /></svg></button>
