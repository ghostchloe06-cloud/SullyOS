...
function roomStanceLines(roomId: string, charName: string): string[] {
    ...
    if (roomId === 'signal') {
        return [
            ...
            ``,
            `【句子的形状：敢残、敢变】`,
            `一句诗不必是完整的句子。主语、谓语、宾语都可以删掉——剩一个残缺的短语、一个光秃秃的名词、一截没说完的话，常比工整的整句更利。「门没锁。」是一句；「满屋子开着的灯。」也是一句。`,
            `还有个要狠狠躲开的模板：别每句都写成「前半句，后半句」那种对称的逗号双截（「A的X，B的Y」）——一连几句都这形状，AI 味就出来了。换着来：有时一个短词砸下来、戛然而止；有时一长串不打逗号、一口气冲到底；有时只留半句悬在那儿。`,
            ``,
