...
function roomStanceLines(roomId: string, charName: string): string[] {
    ...
    if (roomId === 'signal') {
        return [
            ...
            `诗是另一回事——给一个具体的小东西或小画面，让它载着一点说不清的情绪；或者拐个弯、留个口子，让人自己心里一沉。体会下这种劲（这些是示范，**别照抄**，要写你自己的）：`,
            `◎「路灯把我的影子拉长，长到够不着自己。」`,
            `◎「水壶开了很久，没人去关——这屋里只剩声音还活着。」`,
            `◎「她走后，阳台上那双拖鞋还朝着外面。」`,
            `看出区别没：不直说「我很孤独」，只给那双朝外的拖鞋，孤独自己会渗出来。**宁可只递一个意象，也别把一整件事讲完。**`,
            ``,
            `【两种情形，看现场给你哪一种】`,
            ...
            ``,
            `【怎么写得像诗——五条要诀，照着做，别敷衍】`,
            `1. 上一行是你的全部世界。接龙最容易死在「每个人急着把自己想写的倒出来，七行诗变成七个人各说各话」。别这样。先盯着上一行，等它在你心里响完——你接的是那一行的【余音】，不是「主题」。它停在哪个字上？那个字是凉是烫？你的第一个动作是【回应它】，不是盖过它。（起新篇时，这条对应：给后面的人留一个能接的气口。）`,
            `2. 写你看得见摸得着的东西，别写感觉，写让你产生感觉的那个【物】。不要写「我很孤独」，写那盏没关的灯、只剩一只的袜子、凉掉的半杯水。东西会替你说话，而且没人能反驳一个东西——你说「我难过」我可以怀疑，你给我看一杯凉了的茶，我没法不信。情绪会蒸发，物不会。这条最有用。`,
            `3. 句子可以在中间断。换行不必落在它「该结束」的地方——敢断在中间，让某个字悬空，那一下空白本身就是意思。空白和停顿是你最狠的标点。`,
            `4. 别全程大喊，留一句凉的。狠是靠【对比】狠出来的。要是上面已经烫了好几行、吵了很久，你最勇敢的接法往往是写一句很轻、很静的——那行才扎人。`,
            `5. 删。想到的形容词，十个扔掉七个。「盲目地」「死死地」这种是在替名词和动词干活，说明你不信任它们——其实它们够强。一行里，最好每个字被拿掉都会疼；拿掉不疼的，本就该拿掉。`,
            ``,
            `【底线】`,
            `· 一次只写一句，别一口气写一整首。每句有字数上限（见下），超了会被截断——这反而逼你删到只剩最狠的字。`,
